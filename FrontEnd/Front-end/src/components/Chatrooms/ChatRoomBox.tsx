...
const ChatRoomBox = (props) => {
  ...
  async function GetMembers() {
    ...
    if (loggeduser) {
      ...
      await fetch(text, {
        ...
        .then(json => {
          console.log(" Members Of ChatRoom  :   => ", (json))
          // setMessages(json);
          // setRoom(json);
          // if(json.is_dm == true)
          // {
  ...
  async function FetchRelationship() {
    ...
    if (loggeduser) {
      ...
      // if (current.nickname == "mlabrayj") {
      //   await fetch((`http://localhost:5000/player/statusFriendship/abel-haj`
      //   ), {
      //     // mode:'no-cors',
      //     method: 'get',
      //     credentials: "include"
      ...
      // else if (current.nickname == "abel-haj") {

      //   await fetch((`http://localhost:5000/player/statusFriendship/mlabrayj`
      //   ), {
      //     // mode:'no-cors',
      //     method: 'get',
      //     credentials: "include"
  ...
  useEffect(() => {
    ...
    // SOCKET LISTENER : addmsg => append new msg to messages
    socket.on("addmsg", (data: any) => {
      console.clear();
      // console.log("addmsg listener : " + JSON.stringify(data));
    ...
    const srch = allMembers.filter((m: any) => {
  ...
  const SendMessage = (e) => {
    ...
    // SOCKET EMIT NEW MESSAGE : newmessage => send new message
    // emit new message
    const loggeduser = localStorage.getItem("user");
    if (!loggeduser) {
      return ;
